...
namespace QueroDelivery.Service
{
    public class QueroDeliveryService
    {
        ...
        public GenericSimpleResult ReadyToPickup(string token, string placeid, string orderid)
        {
            var result = new GenericSimpleResult();
            try
            {
        ...
        public GenericSimpleResult Dispatch(string token, string placeid, string orderid)
        {
            var result = new GenericSimpleResult();
            try
            {
                var url = string.Format("{0}{1}/{2}/{3}?placeId={4}", Constants.URL_BASE, Constants.URL_ORDER, orderid, Constants.URL_ORDER_DISPATCH, placeid);
                var client = new RestClient(url);
                var request = new RestRequest(Method.POST);
                request.AddHeader("Authorization", "Basic " + token);
                IRestResponse response = client.Execute(request);
