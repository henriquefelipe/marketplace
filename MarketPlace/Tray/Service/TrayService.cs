...
namespace Tray.Service
{
    ...
    {        
        ...
        public GenericResult<paymentMethodsResult> PaymentsMethods(string access_token)
        {
            var result = new GenericResult<paymentMethodsResult>();

            System.Net.ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;
            var client = new RestClient(_urlBase + Constants.URL_PAYMENTS_METHODS_ACTIVE + $"?access_token={access_token}");
            var request = new RestRequest(Method.GET);
            IRestResponse response = client.Execute(request);

            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                result.Result = JsonConvert.DeserializeObject<paymentMethodsResult>(response.Content);
                result.Success = true;
            }
            else
            {
